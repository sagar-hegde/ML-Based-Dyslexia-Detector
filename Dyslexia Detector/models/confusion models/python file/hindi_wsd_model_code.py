"""
Hindi Word Sense Disambiguation (WSD) Model
Uses multilingual BERT optimized for Hindi text
...
# ========================================
# STEP 3: Load Your Hindi Dataset
# ========================================
print("\n" + "="*60)
print("📂 Upload your Hindi CSV file")
print("="*60)
...
# ========================================
# STEP 5: Dataset Class for Hindi
# ========================================
class HindiWSDDataset(Dataset):
...
# ========================================
class HindiBERTForWSD(nn.Module):
    def __init__(self, model_name, dropout_rate=0.3):
        super(HindiBERTForWSD, self).__init__()
...
# ========================================
# STEP 7: Initialize Hindi Model
# ========================================
print("\n" + "="*60)
print("🤖 Initializing Hindi BERT model...")
print("="*60)
...
model = HindiBERTForWSD(MODEL_NAME, dropout_rate=0.4)
model.to(device)
...
train_dataset = HindiWSDDataset(train_contexts, train_glosses, train_labels, tokenizer)
test_dataset = HindiWSDDataset(test_contexts, test_glosses, test_labels, tokenizer)
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_accuracy:
        ...
        patience_counter = 0
        torch.save(model.state_dict(), 'best_hindi_wsd_model.pt')
...
model.load_state_dict(torch.load('best_hindi_wsd_model.pt'))
...
print("\n🔽 Downloading best_hindi_wsd_model.pt...")
files.download('best_hindi_wsd_model.pt')
print("✅ Model downloaded successfully!")

print("\n📦 To use this model later:")
print("  1. Upload the .pt file to Colab")
print("  2. Load with: model.load_state_dict(torch.load('best_hindi_wsd_model.pt'))")

# ========================================
# STEP 14: Prediction Function
# ========================================
def predict_hindi_word_sense(context, gloss, target_word):
    model.eval()
...
for context in sample_contexts:
    ...
    for idx, row in context_data.iterrows():
        ...
        prediction, confidence = predict_hindi_word_sense(context, gloss, target_word)
...
print("\n" + "="*60)
print("✅ Hindi WSD model training complete!")
print("="*60)
print("\n💡 Model has been downloaded to your computer!")
print("📁 File: best_hindi_wsd_model.pt")